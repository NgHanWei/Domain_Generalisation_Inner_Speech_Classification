...
with torch.no_grad():
    ...
    reconstruction, mu, logvar = model(X_test[:1])
    test_bce_loss = recon_loss(reconstruction,X_test[:1])
    print(test_bce_loss)
    ...
    for batch in tqdm(range(0,len(data_load_check))):    
        ...
        if batch_bce_loss < test_bce_loss*1.5:
            print(batch_bce_loss)
...
torch.save(base_model_param, pjoin(
    './results/', 'model_subj{}.pt'.format(targ_subj)))
model.epochs_df.to_csv(
    pjoin('./results/', 'epochs_subj{}.csv'.format(targ_subj)))
